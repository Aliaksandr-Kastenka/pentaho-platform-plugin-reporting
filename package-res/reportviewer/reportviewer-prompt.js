...
define([
  ...
  return function() {
    return logged({
      ...
      _buildReportContentOptions: function(renderMode, paramName) {
        var options = util.getUrlParameters();
        $.extend(options, this.api.operation.getParameterValues(paramName));
      ...
      createPromptPanel: function() {
        this.api.operation.render(function(api, callback, name) {
          ...
          this.fetchParameterDefinition(paramDefnCallback.bind(this), this.mode, name);
        }.bind(this));
       },
       ...
       */
      fetchParameterDefinition: function(callback, promptMode, name) {
        var me = this;
        ...
        var param;
        if(this.api.operation._getPromptPanel().paramDefn) {
          for(var i=0; i<this.api.operation._getPromptPanel().paramDefn.getParameterGroup("parameters").parameters.length; i++) {
            if(name == this.api.operation._getPromptPanel().paramDefn.getParameterGroup("parameters").parameters[i].name) {
              param = this.api.operation._getPromptPanel().paramDefn.getParameterGroup("parameters").parameters[i];
              break;
            }
          }
        }

        //at the very first time(INITAL mode) param is undefined, send request and parse all parameter dependencies/dependents from XML-response
        if((typeof param == 'undefined' || !param.canBeValidatedOnClientSide)) {
          ...
          var selectParamsWithDependencies;
          if (promptMode == 'MANUAL' || promptMode == 'USERINPUT') {
            selectParamsWithDependencies = true;
          }
          var options = me._buildReportContentOptions(this._getParameterDefinitionRenderMode(promptMode), selectParamsWithDependencies ? name : undefined);
          ...
          // no server request will be made, work with old parameter definition
          var untrustedValue = this.api.operation._getPromptPanel().getParameterValue(this.api.operation._getPromptPanel().getParameterName(name));
          var paramDefn = $.extend(true, {}, this.api.operation._getPromptPanel().getParamDefn()); // clone previous(old) paramDefn
          this.api.util.validateSingleParameter(paramDefn, name, untrustedValue);
